...
public final class Store: ObservableObject {
    ...
    @Published
    public private(set) var peripherals = [NativeCentral.Peripheral: InkbirdAdvertisement]()
    ...
    public func scan(duration: TimeInterval? = nil) async throws {
        ...
        guard bluetoothState == .poweredOn else {
            throw InkbirdAppError.bluetoothUnavailable
        }
    ...
    private func found(_ scanData: ScanData<NativeCentral.Peripheral, NativeCentral.Advertisement>) -> Bool {
        guard let advertisement = InkbirdAdvertisement(scanData.advertisementData) else {
            return false
        }
